...
from data3d.suncg_utils.suncg_metas import SUNCG_METAS as  STANFORD_METAS
...
SuncgTorch_PATH = os.path.join(CUR_DIR, 'StanfordTorch')
ELEMENTS_IDS = {'xyz':[0,1,2], 'color':[3,4,5], 'normal':[6,7,8]}

POINTS_NUM = int(1e5)

FORCELY_SET_YAW_0 = True
...
class STANFORDDataset(torch.utils.data.Dataset):
  def __init__(self, split, cfg):
    ...
    dimension=3
    self.dset_metas = STANFORD_METAS(cfg.INPUT.CLASSES)
  ...
  def __getitem__(self, index):
        ...
        fn = self.files[index]
        print(f'Loading {fn} in STANFORDDataset')
        ...
        for obj in bboxes_dic_i_0:
          if ('all' in objects_to_detect) or (obj in objects_to_detect):
            ...
            # To make the yaw as 0
            bboxes_dic_i_0[obj][:,-1] = np.pi / 2
            bboxes_dic_i_0[obj] = bboxes_dic_i_0[obj][:,[0,1,2,4,3,5,6]]
