...
def rotate_nms_gpu(dets, nms_overlap_thresh, device_id=0):
    ...
    with stream.auto_synchronize():
        ...
        rotate_nms_kernel[blockspergrid, threadsPerBlock, stream](
            boxes_num, nms_overlap_iouthresh, boxes_dev, mask_dev)
        mask_dev.copy_to_host(mask_host, stream=stream)
    num_out = nms_postprocess(keep_out, mask_host, boxes_num)
    keep = keep_out[:num_out]
    return list(order[keep])
...
def devRotateIoUEval(rbox1, rbox2, criterion=-1):
    '''
    rbox1: [4] [center_x, center_y, size_y, size_x]
    rbox2: [4] [center_x, center_y, size_y, size_x]
    '''
    area1 = rbox1[2] * rbox1[3] # anchor
    area2 = rbox2[2] * rbox2[3] # target
    area_inter = inter(rbox1, rbox2)

    length1_x = rbox1[0] + rbox1[2] * 0.5
    length1_y = rbox1[1]
    width1_x = rbox1[0]
    width1_y = rbox1[1]+rbox1[3]*0.5
    length2_x = rbox2[0]+rbox2[2]*0.5
    length2_y = rbox2[1]
    width2_x = rbox2[0]
    width2_y = rbox2[1]+rbox2[3]*0.5
    ...
    Dlength = ((length1_x - length2_x) ** 2 + (length1_y - length2_y) ** 2) ** 0.5
    Dwidth = ((width1_x - width2_x) ** 2 + (width1_y - width2_y) ** 2) ** 0.5
    # (rbox1[0],rbox1[1]) = 中心点坐标（x，y） , rbox1[2]=长，rbox1[3]=宽，area_inter=面积交集
    # （rbox1[0]+rbox1[2]*0.5，rbox1[1]）=anchor右边中点（x，y），（rbox1[0]，rbox1[1]+rbox1[3]*0.5）=anchor上边中点（x，y）
    # （rbox2[0]+rbox2[2]*0.5，rbox2[1]）=target右边中点（x，y），（rbox2[0]，rbox2[1]+rbox2[3]*0.5）=target上边中点（x，y）
    # 长边=rbox1[2] if rbox1[2] > rbox1[3] else长边=rbox1[3]
    if criterion == -1:
        return area_inter / (area1 + area2 - area_inter)
    elif criterion == 3:
        # 加权IOU = IOU * weighIOU + DIOU * weightDIOU + AIOU * weightAIOU
        weighIOU, weightDIOU, weightAIOU = 0,0.1,0.1
        ...
        return IOU * weighIOU + DIOU * weightDIOU + AIOU * weightAIOU
    elif criterion == 4:
        # Midpoint distance

        return 2-(Dlength + Dwidth + 1.5*distance_of_central)/1
        # return 2 - (Dlength + Dwidth + 1.5 * distance_of_central) / 3
    elif criterion == 5:
        # 比较长边，中心点，长宽比
        if rbox1[2] >= rbox1[3]:
            long1 = rbox1[2]
        ...
        if rbox2[2] >= rbox2[3]:
            ...
            long2 = rbox2[3]
        AIOU =  (4 / (math.pi ** 2)) * (math.atan(rbox1[2] / rbox1[3]) - math.atan(rbox2[2] / rbox2[3])) ** 2
        return 1 - (((long1-long2)**2)**0.5+0 + distance_of_central) / 0.5 + 0.2*AIOU
        # return 2-(((rbox1[2]-rbox2[2])**2)**0.5 + ((rbox1[3]-rbox2[3])**2)**0.5 + distance_of_central)/1
    elif criterion == 6:
        #
        m=abs(rbox1[2]-rbox2[2])+abs(rbox1[3]-rbox2[3])+distance_of_central
        return 1-m/0.7
    elif criterion == 0:
